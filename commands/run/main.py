...
def main(
    ...
    def get_start_nextflow_command(quote_pipeline_arguments: bool) -> List[str]:
        command = build_start_nextflow_analysis_cmd(
            ...
            [repo / conf for conf in config.general_settings.nextflow_configs],
        )
        return command
...
def do_repo_checkout(
    ...
    if checkout.startswith(remote_prefix):
        ...
        logger.warning(
            'Detected checkout of a remote branch (origin/my-branch). Trimming the remote (i.e. "origin").'
        )
        logger.warning(
            f"checkout after trimming: {checkout_branch}, remote checkout: {remote_checkout}"
        )
